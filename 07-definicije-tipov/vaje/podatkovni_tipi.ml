...
let euro_to_dollar (Euro x) = Dollar (x *. 1.03)
let dollar_to_euro (Dollar x) = Euro (x *. 0.97)
...
let to_pound obj =
  match obj with
  | Yen x -> Pound (x *. 0.0059)
  | Pound x -> Pound x
  | Krona x -> Pound (x *. 0.079)
...
let intbool_list_primer = Int(5, Bool(true, Bool(false, Int(7, Prazen))))
(*----------------------------------------------------------------------------*]
...
let rec intbool_map f_int f_bool ib_list =
  match ib_list with
  | Prazen -> Prazen
  | Int (x, seznam) -> Int (f_int x , intbool_map f_int f_bool seznam)
  | Bool (x, seznam) -> Bool (f_bool x , intbool_map f_int f_bool seznam)
...
let intbool_reverse seznam =
  let rec aux acc seznam =
    match seznam with
    | Prazen -> acc
    | Int (x, seznam) -> aux (Int(x,acc)) seznam
    | Bool (x, seznam) -> aux (Bool(x,acc)) seznam 
  in aux Prazen seznam
...
let intbool_separate ib_list = 
  let rec aux list1 list2 ib_list =
    match ib_list with
    | Prazen -> (list1, list2)
    | Int (x, seznam) -> aux (x::list1) list2 seznam
    | Bool (x, seznam) -> aux list1 (x::list2) seznam
  in aux [] [] ib_list
